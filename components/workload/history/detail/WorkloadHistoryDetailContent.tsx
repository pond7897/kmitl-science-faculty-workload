...
function normalizeDayOfWeek(dayOfWeek: string): DayKey | null {
  ...
  if (
    ["thu", "thur", "thurs", "thursday", "พฤหัสบดี", "วันพฤหัสบดี"].includes(
      normalized,
    )
  ) {
    return "thursday";
...
function formatSessionRanges(
  sessions: WorkloadHistorySessionSlot[],
  emptyLabel: string,
) {
...
function WeekSelectionCard({
  ...
  const maxWeek = Math.max(
    totalWeeks,
    weeks.reduce(
      (currentMax, week) => Math.max(currentMax, week.weekNumber),
      0,
    ),
  );
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isActive &&
                    isExternal &&
                    "border-[#F27F0D]/30 bg-[#FFF7ED] text-[#9A3412]",
                  !isActive &&
                    "border-border bg-[#F8F7F5] text-[#1E293B] dark:bg-[#2B2421] dark:text-[#DCCFC7]",
                )}
              >
                ...
                {isExternal && (
                  <span className="mt-1 max-w-full truncate text-[10px] leading-4">
                    {week?.instructorName ||
                      t("WorkloadHistoryDetail.externalSpeaker")}
                  </span>
                )}
              </div>
            );
...
export function WorkloadHistoryDetailContent({
  ...
  const scheduleSessions = [
    ...detail.theorySessions.map((session) => ({
  ...
  const sectionLabel = isEnglish
    ? `Section ${detail.section}`
    : `หมู่เรียน ${detail.section}`;
    ...
    <div className="flex w-full flex-col gap-4 pb-8 pt-1 md:gap-5">
      <div className="flex justify-start">
        <Button
          asChild
          variant="outline"
          className="gap-2 border-[#F27F0D]/20 text-[#F27F0D] hover:bg-orange-50 hover:text-[#F27F0D]"
        >
            ...
            <div className="grid gap-4 xl:grid-cols-[minmax(0,1fr)_340px]">
              <div className="space-y-4">
                <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
                  ...
                  <div className="mt-4 grid gap-3 md:grid-cols-2">
                    <DetailField
                      label={t("WorkloadHistoryDetail.faculty")}
                      value={detail.faculty}
                    />
                    <DetailField
                      label={t("WorkloadHistoryDetail.major")}
                      value={detail.major}
                    />
                    <DetailField
                      label={t("WorkloadHistoryDetail.yearLevel")}
                      value={detail.yearLevel}
                    />
                    <DetailField
                      label={t("WorkloadHistoryDetail.section")}
              ...
              <div className="h-fit rounded-xl border border-[#F27F0D]/20 bg-[#F27F0D]/5 p-4 shadow-sm xl:sticky xl:top-5">
                ...
                <div className="mt-4 space-y-4">
                  <SidebarField
                    label={t("WorkloadHistoryDetail.courseName")}
                    value={detail.courseName}
                  />
                  <SidebarField
                    label={t("WorkloadHistoryDetail.courseCode")}
                    value={detail.courseCode}
                  />

                  <div className="grid grid-cols-2 gap-4">
                    <SidebarField
                      label={t("WorkloadHistoryDetail.credits")}
                      value={detail.credits}
                    />
                    <SidebarField
                      label={t("WorkloadHistoryDetail.degree")}
                      value={detail.degree}
                    />
                    <SidebarField
                      label={t("WorkloadHistoryDetail.theoryTime")}
                      value={theoryTimeText}
                    />
                    <SidebarField
                      label={t("WorkloadHistoryDetail.labTime")}
                      value={labTimeText}
                    />
                    <SidebarField
                      label={t("WorkloadHistoryDetail.theoryHours")}
                      value={formatHours(
                        detail.totalTheoryHours,
                        isEnglish,
                        hoursUnit,
                        emptyLabel,
                      )}
              ...
              <Table className="min-w-[760px]">
                <TableHeader>
                  <TableRow className="bg-muted/50 hover:bg-muted/50">
                    <TableHead>
                      {t("WorkloadHistoryDetail.historyAction")}
                    </TableHead>
                    <TableHead>
                      {t("WorkloadHistoryDetail.historyDate")}
                    </TableHead>
                    <TableHead>
                      {t("WorkloadHistoryDetail.historyStatus")}
                    </TableHead>
                    <TableHead>
                      {t("WorkloadHistoryDetail.historyActor")}
                    </TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                      ...
                      return (
                        ...
                        >
                          ...
                          <TableCell className="text-foreground">
                            {process.actorName ||
                              t("WorkloadHistoryDetail.actorFallback")}
                          </TableCell>
                        </TableRow>
                      );
                    })
